...
func (s *LaunchScreenServiceImpl) CreateImage(stream launch_screen.LaunchScreenService_CreateImageServer) (err error) {
	...
	req, err := stream.Recv()
	if err != nil {
		logger.WithCtx(stream.Context()).Infof("LaunchScreen.CreateImage recv request: %v", err)
	...
	for i := 0; i < int(req.BufferCount); i++ {
		...
		if err != nil {
			logger.WithCtx(stream.Context()).Infof("LaunchScreen.CreateImage recv file: %v", err)
			resp.Base = base.BuildBaseResp(err)
			return stream.SendAndClose(resp)
		}
	...
	if err != nil {
		logger.WithCtx(stream.Context()).Infof("LaunchScreen.CreateImage: %v", err)
		resp.Base = base.BuildBaseResp(err)
		return stream.SendAndClose(resp)
	}
	base.OK(resp)
...
func (s *LaunchScreenServiceImpl) ChangeImage(stream launch_screen.LaunchScreenService_ChangeImageServer) (err error) {
	...
	req, err := stream.Recv()
	if err != nil {
		logger.WithCtx(stream.Context()).Infof("LaunchScreen.ChangeImage recv request: %v", err)
		resp.Base = base.BuildBaseResp(err)
		return stream.SendAndClose(resp)
	}
	...
	for i := 0; i < int(req.BufferCount); i++ {
		fileReq, err := stream.Recv()
		if err != nil {
			logger.WithCtx(stream.Context()).Infof("LaunchScreen.ChangeImage recv file: %v", err)
			resp.Base = base.BuildBaseResp(err)
			return stream.SendAndClose(resp)
		}
		req.Image = bytes.Join([][]byte{req.Image, fileReq.Image}, []byte(""))
	...
	if err != nil {
		logger.WithCtx(stream.Context()).Infof("LaunchScreen.ChangeImage: %v", err)
		resp.Base = base.BuildBaseResp(err)
		return stream.SendAndClose(resp)
	}
	base.OK(resp)
...
func (s *LaunchScreenServiceImpl) AddImagePointTime(ctx context.Context, req *launch_screen.AddImagePointTimeRequest) (
	...
	resp.Base = base.BuildBaseResp(err)
	if err != nil {
		logger.WithCtx(ctx).Infof("LaunchScreen.AddImagePointTime: %v", err)
	}
	return resp, nil
}
